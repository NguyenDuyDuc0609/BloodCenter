...
namespace BloodCenter.Controllers
{
    ...
    public class HospitalController : ControllerBase
    {
        ...
        [HttpPost("GetRequestBlood")]
        [Authorize(Roles = "Hospital")]
        public async Task<IActionResult> GetRequestBlood(int pageNumber, int pageSize, int status)
        {
            _result = await _hospital.GetRequestBlood(pageNumber, pageSize, status);
            return Ok(_result);
        }
        [HttpPost("AcceptRequestBlood")]
        [Authorize(Roles = "Hospital")]
        public async Task<IActionResult> AcceptRequestBlood([FromBody] string requestId)
        {
            _result = await _hospital.AcceptRequestBlood(Request.Headers["Authorization"], requestId);
            return Ok(_result);
        }
    }
}
